...
public class RetrieveProductsController {
    ...
    public String getAllProducts(@RequestParam(defaultValue = "1") int page,
                                 ...
                                 Model model) {
        log.info("모든 제품 목록 요청 - 페이지: {}, 사이즈: {}", page, size);
        try {
            RetrieveProductsResponse response = productService.getAllProducts(page, size);
    ...
    public String getProductsByCategory(@RequestParam(defaultValue = "1") int page,
                                        ...
                                        Model model) {
        log.info("카테고리별 제품 목록 요청 - 카테고리 ID: {}, 페이지: {}, 사이즈: {}", categoryId, page, size);
        try {
            RetrieveProductsResponse response = productService.getProductsByCategory(categoryId, page, size);
            model.addAttribute("productsResponse", response);
            model.addAttribute("products", response.getProducts());
            return "productList";
        } catch (Exception e) {
